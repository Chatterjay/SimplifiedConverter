...
def main():
    ...
    while True:
        ...
        conversion_type = input("请输入转换类型 ('zh-cn' 用于繁体转简体, 'zh-hk' 用于简体转繁体)：")
        if conversion_type not in ['zh-cn', 'zh-hk']:
            print("无效的转换类型，请输入 'zh-cn' 或 'zh-hk'")
            continue
        ...
        handle_path_input(path, conversion_map[conversion_type])
